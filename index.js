...
// transfer diler
app.get('/api/transfer/:diler_code', (req, res) => {
  const diler_code = md5(req.params.diler_code);
  
  pool.query('SELECT * FROM `transfer-diler` WHERE `diler_code` = ? ',
    [diler_code], (err, result) => {
      if (err) {
          res.status(500).json({ error: 'Ошибка выполнения запроса к базе данных' });
          console.error(err);
          return;
